;;;
;;; If there are more than 1 elements of the number, it is dead robots
;;; Dead robots are '#' on console, which don't move and kill other robots when overrapped.
;;;
...
(defun robots ()
  (loop named main
     ...
     when (loop for mpos in monsters
	     always (> (count mpos monsters) 1))  ;; All the points where a monster is have 2 or more monsters
     return 'player-wins
     do (format t
		"~%|~{~<|~%|~,65:;~A~>~}|"
		(loop for p
